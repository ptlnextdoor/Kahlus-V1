...
class EvidenceBundleArtifactTests(unittest.TestCase):
    def _create_a100_evidence_fixture(
        ...
        files = {
            ...
            run / "eval_audit.json": '{"passed":true}\n',
            run / "prepared_baseline_suite.json": "{}\n",
            run / "seed_aggregate.json": "[]\n",
            run / "seed_aggregate.csv": "task_id,model_id,metric,mean,std,ci_low,ci_high,n_seeds\n",
            run / "baseline_failures.json": "[]\n",
            run / "paper_mode_gate.json": '{"passed":true}\n',
            run / "evidence_gate.json": '{"passed":false,"scientific_claim_allowed":false}\n',
            run / "diagnostic_report.md": "# Diagnostic Report\n",
            run / "pair_operator_ablation.csv": "task_id,ablation,status\n",
            run / "uncertainty_calibration.csv": "task_id,status,reason\n",
            run / "leakage_demo.json": '{"scientific_claim_allowed":false}\n',
            run / "LEAKAGE_DEMO.json": '{"scientific_claim_allowed":false}\n',
            run / "classification_leakage_demo.json": '{"scientific_claim_allowed":false}\n',
            run / "CLASSIFICATION_LEAKAGE_DEMO.json": '{"scientific_claim_allowed":false}\n',
            run / "classification_leakage_demo.csv": "seed,split_id,metric,value\n",
            run / "classification_leakage_demo.md": "# Classification Leakage Demo\n",
            run / "classification_leakage_figure.json": "{}\n",
            run / "classification_leakage_claim_gate.json": '{"bad_split_claim_allowed":false}\n',
            run / "split_comparison.csv": "seed,split_id,subject_overlap\n",
            run / "identity_probe.json": '{"scientific_claim_allowed":false}\n',
            run / "IDENTITY_PROBE.json": '{"scientific_claim_allowed":false}\n',
            run / "EEG_MODEL_CARD.md": "# EEG Model Card\n",
            run / "LEAKAGE_AUDIT.json": '{"passed":true}\n',
            run / "CLAIM_GATE.json": '{"passed":true}\n',
            run / "BASELINE_RANKING.csv": "task_id,model_id,metric,value,rank\n",
    ...
    def test_package_a100_evidence_bundle_excludes_checkpoints_and_secrets(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            required = {
                ...
                "run/prepared_baseline_suite.json",
                "run/seed_aggregate.json",
                "run/seed_aggregate.csv",
                "run/baseline_failures.json",
                "run/paper_mode_gate.json",
                "run/evidence_gate.json",
                "run/diagnostic_report.md",
                "run/pair_operator_ablation.csv",
                "run/uncertainty_calibration.csv",
                "run/leakage_demo.json",
                "run/classification_leakage_demo.json",
                "run/classification_leakage_demo.csv",
                "run/classification_leakage_demo.md",
                "run/classification_leakage_figure.json",
                "run/classification_leakage_claim_gate.json",
                "run/split_comparison.csv",
                "run/identity_probe.json",
                "run/EEG_MODEL_CARD.md",
                "run/LEAKAGE_AUDIT.json",
                "run/CLAIM_GATE.json",
                "run/BASELINE_RANKING.csv",
