...
| C005 | NFC is a continuous neural field or neural operator | Coordinate conditioning exists, but no resolution-transfer, discretization-invariance, or function-space operator evidence exists | UNSUPPORTED | "Experimental coordinate-conditioned latent-tensor dynamics architecture" |
...
| C012 | Uncertainty is calibrated | predictive scale is trained by Gaussian NLL; held-out calibration and coverage remain unavailable | IMPLEMENTED_BUT_NOT_VALIDATED | "Produces a likelihood-trained predictive scale; calibration is unvalidated" |
