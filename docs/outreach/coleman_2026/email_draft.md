...
3. **Parallel honesty (F2–F5).** Isolated forecasting audit: GRU does not beat trivial baselines (F2). Interoception scout on Sleep-EDF peripheral channels: honest negative (F3). Powered Passive PCI on full Sleep-EDF cassette (78 subjects, n_boot=2000): complexity block **hurts** vs spectral baseline — wake RFS −0.330 bits, CI excludes zero on the negative side (F4). Same result replicated under a different etiology — full 21-subject OpenNeuro propofol cohort: RFS −0.2072 bits, CI excludes zero (F5). We are **not** leading with forecasting-skill or consciousness claims; these are proof the courtroom bites across independent cohorts.
