...
**Paper-identity lock (2026-07-24):** this submission is complete and submittable *as-is* on the strength of the arena artifact plus F0–F5's powered negatives — no positive result is required for the paper to stand. F6 (NSRR MESA, real cohort) is upside, not a prerequisite: MESA/SHHS credentialed access has been applied for; if it lands before the deadline, F6 runs real and either strengthens the paper with a scoped positive or extends the powered-negative set. If it does not land in time, the paper ships without it. This line exists so the submission does not sit in limbo waiting on a manual, externally-gated download.
...
3. **Arena bites (F2–F5)** — isolated forecast negative, interoception scout negative, Passive PCI powered negative, propofol cross-etiology probe (full 21/21-subject cohort, RFS −0.2072, CIs exclude 0).
