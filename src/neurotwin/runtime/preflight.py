...
from neurotwin.config_types import as_prepared_training_config_input, resolve_prepared_config
...
def run_cluster_preflight(
    ...
    config = as_prepared_training_config_input(load_config(config_file))
    resolved_config = resolve_prepared_config(config, require_manifests=False)
    event_manifest = resolved_config.event_manifest
    split_manifest = resolved_config.split_manifest
    ...
    if needs_window_audit:
        if event_path is not None and split_path is not None and event_path.exists() and split_path.exists():
            audit = audit_prepared_eval_inputs(
                event_path,
                split_path,
                window_length=resolved_config.window_length,
