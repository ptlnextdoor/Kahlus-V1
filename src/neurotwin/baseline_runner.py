...
def _run_single_model_selected(
    ...
    single fitted result and the policy is ``fitted``.
    """
    ...
    if model_id == "nfc":
        # Same honest skip as the final-step path (raises so the caller records the failure).
        _run_single_model(model_id, task, train_steps, seed)

    if model_id == "mlp":
        set_global_seed(seed)
        model: nn.Module = TorchMLPBaseline(task.x_train.shape[1] * channels_in, channels_out)
        ...
        xv = torch.tensor(_flatten_window(task.x_val), dtype=torch.float32)
        yv = torch.tensor(np.asarray(task.y_val, dtype=np.float64), dtype=torch.float32)
        xe = torch.tensor(_flatten_window(task.x_test), dtype=torch.float32)
        sequence = False
    elif model_id in {"transformer", "ssm_fallback"}:
        set_global_seed(seed)
        ...
        yt = torch.tensor(np.asarray(task.y_train, dtype=np.float64), dtype=torch.float32)
        xv = torch.tensor(np.asarray(task.x_val, dtype=np.float64), dtype=torch.float32)
        yv = torch.tensor(np.asarray(task.y_val, dtype=np.float64), dtype=torch.float32)
        xe = torch.tensor(np.asarray(task.x_test, dtype=np.float64), dtype=torch.float32)
        sequence = True
    else:
        raise RuntimeError(f"unknown baseline model_id: {model_id!r}")

    best_state, final_state, best_step = _train_torch_select(
        model, xt, yt, xv, yv, train_steps, sequence=sequence, eval_every=eval_every
    )
    pred_best = _predict_state(model, best_state, xe, sequence)
    pred_final = _predict_state(model, final_state, xe, sequence)
    return {
        "pred_best_val": pred_best,
