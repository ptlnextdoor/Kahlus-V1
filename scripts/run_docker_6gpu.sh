...
fi
{
  ...
  printf 'A100_RUN_PAPER_MODE_IN_FULL=%s\n' "$A100_RUN_PAPER_MODE_IN_FULL"
  printf 'A100_PAPER_MODE_EVAL_DIR=%s\n' "${A100_PAPER_MODE_EVAL_DIR:-}"
...
echo "a100_run_paper_mode_in_full=$A100_RUN_PAPER_MODE_IN_FULL"
...
docker run --rm "${DOCKER_TTY[@]}" --gpus "\"device=${HOST_GPU_IDS}\"" \
  ...
  -e A100_PAPER_MODE_EVAL_DIR="${A100_PAPER_MODE_EVAL_DIR:-}" \
