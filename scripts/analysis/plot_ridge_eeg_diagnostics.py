...
class RidgeFigureData:
    ...
    model_pred_test: np.ndarray | None
    sfreq: float
    signal_unit: str
    context_samples: int
...
def load_data(args: argparse.Namespace) -> RidgeFigureData:
    ...
    if args.npz is None:
        ...
        if "kahlus_prediction_test" in data:
            model_pred_test = _as_3d(data["kahlus_prediction_test"], args.time_length, args.n_channels, "kahlus_prediction_test")
            if model_pred_test.shape != y_test.shape:
                raise ValueError(f"kahlus_prediction_test shape {model_pred_test.shape} != y_test shape {y_test.shape}")
...
def stamp_axis(ax, data: RidgeFigureData) -> None:
    ...
    elif data.protocol_id == "kahlus.forecast.v1_overlap":
...
def figure_mentor_context_target(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    ...
    axes[0].set_title(
        f"BNCI2014-001 historical one-sample-shift task: {max(0, data.context_samples - data.forecast_horizon_samples)}/"
...
def figure_mentor_baselines_autocorr(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    ...
    if data.model_pred_test is not None:
        ax.plot(t, data.model_pred_test[idx, :, channel], color=PURPLE, lw=1.05, label="provided Kahlus prediction")
    else:
        ax.text(
            0.02,
            0.05,
    ...
    for channel in channels:
        signal = data.x_train[:, :, channel].reshape(-1)
        signal = signal - signal.mean()
        denominator = float(np.dot(signal, signal)) or 1.0
        autocorr = [float(np.dot(signal[:-lag or None], signal[lag:]) / denominator) for lag in range(max_lag + 1)]
        ax.plot(lags, autocorr, lw=1.0, label=data.channel_names[channel])
...
def figure_prediction(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    ...
    for ax, ch in zip(axes, channels):
        ...
        if data.model_pred_test is not None:
            ax.plot(t, data.model_pred_test[idx, :, ch], color=PURPLE, lw=1.05, alpha=0.88, label="provided Kahlus prediction")
    ...
    if data.model_pred_test is None:
...
def main() -> None:
    ...
    if args.mentor_two_figures:
        figure_mentor_context_target(data, channels, args.out)
        figure_mentor_baselines_autocorr(data, channels, args.out)
        write_readme(args.out, data, ["only the two compact mentor-facing diagnostics were requested"])
