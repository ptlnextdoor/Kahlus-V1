...
from neurotwin.data.forecast_contract import FORECAST_PROTOCOL_V1_OVERLAP  # noqa: E402
from neurotwin.eeg_v1.window_evidence import (  # noqa: E402
    fresh_atomic_output_directory,
...
class RidgeFigureData:
    ...
    persistence_pred_test: np.ndarray
...
def load_data(args: argparse.Namespace) -> RidgeFigureData:
    ...
    if args.npz is None:
        ...
        persistence_pred_test = x_test.copy()
        train_record_ids = np.asarray([f"synthetic-{index}" for index in range(x_train.shape[0])], dtype="U")
        signal_unit = "a.u."
        context_samples = int(x_train.shape[1])
        forecast_horizon_samples = int(x_train.shape[1])
        protocol_id = "synthetic_schematic"
        schematic = True
        provenance["source"] = "synthetic schematic generated by plotting script"
    else:
        data = np.load(args.npz, allow_pickle=False)
        if "kahlus_prediction_test" in data:
            raise ValueError(
                "kahlus_prediction_test is unsupported without complete checkpoint and prediction provenance"
            )
        ...
        signal_unit = str(data["signal_unit"].item()) if "signal_unit" in data else "unknown"
        context_samples = int(data["context_samples"]) if "context_samples" in data else int(x_train.shape[1])
        forecast_horizon_samples = int(data["forecast_horizon_samples"]) if "forecast_horizon_samples" in data else context_samples
        protocol_id = str(data["protocol_id"].item()) if "protocol_id" in data else "unknown"
        ...
        persistence_pred_test = (
            _as_3d(data["persistence_prediction_test"], args.time_length, args.n_channels, "persistence_prediction_test")
            if "persistence_prediction_test" in data
            else x_test.copy()
        )
        if persistence_pred_test.shape != y_test.shape:
            raise ValueError(
                f"persistence_prediction_test shape {persistence_pred_test.shape} != y_test shape {y_test.shape}"
            )
        if "train_record_ids" not in data:
            raise ValueError("real diagnostic arrays require train_record_ids for boundary-safe ACF")
        train_record_ids = np.asarray(_npz_strings(data, "train_record_ids", []), dtype="U")
    ...
    return RidgeFigureData(
        ...
        persistence_pred_test=persistence_pred_test,
        train_record_ids=train_record_ids,
        sfreq=sfreq,
        signal_unit=signal_unit,
        context_samples=context_samples,
        forecast_horizon_samples=forecast_horizon_samples,
...
def stamp_axis(ax, data: RidgeFigureData) -> None:
    if data.schematic:
        text = "SCHEMATIC - NOT BENCHMARK EVIDENCE"
    elif data.protocol_id == FORECAST_PROTOCOL_V1_OVERLAP:
        text = "HISTORICAL OVERLAP DIAGNOSTIC - NOT CLAIM EVIDENCE"
    else:
        text = "BENCHMARK-DERIVED FIGURE"
    ax.text(
        0.0,
        -0.22,
        f"{text}. Prediction source: {data.prediction_source}.",
...
def figure_input_target(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    ...
    ty = (np.arange(y.shape[1]) + data.forecast_horizon_samples) / data.sfreq
    ...
    for row, ch in enumerate(channels):
        offset = row * 2.6
        ax.plot(ty, y[idx, :, ch] / scale + offset, color=RED, lw=1.0, ls="--", alpha=0.82)
        ax.plot(tx, x[idx, :, ch] / scale + offset, color=BLUE, lw=1.08)
    ...
    ax.axvspan(ty[0], ty[-1], color=RED, alpha=0.08, label="shifted target Y_t")
    ax.set_title(
        "Historical shifted-window EEG diagnostic: "
        f"target offset {data.forecast_horizon_samples} sample(s), shared context "
        f"{max(0, data.context_samples - data.forecast_horizon_samples)}/{data.context_samples}"
    )
    ax.set_xlabel("Time (s)")
    ax.set_ylabel(f"EEG ({data.signal_unit}; traces offset)")
    ax.set_yticks([])
    ...
    fig.colorbar(im, ax=ax2, fraction=0.046, label=f"amplitude ({data.signal_unit})")
...
def figure_mentor_context_target(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    """Render the compact, literal window diagram requested for mentor review."""

    idx = 0
    tx = np.arange(data.x_test.shape[1]) / data.sfreq
    ty = (np.arange(data.y_test.shape[1]) + data.forecast_horizon_samples) / data.sfreq
    fig, axes = plt.subplots(len(channels), 1, figsize=(7.16, 1.25 * len(channels) + 1.0), sharex=True, constrained_layout=True)
    if len(channels) == 1:
        axes = [axes]
    for position, (ax, channel) in enumerate(zip(axes, channels)):
        ax.plot(tx, data.x_test[idx, :, channel], color=BLUE, lw=1.25, label="context X" if position == 0 else None)
        ax.plot(ty, data.y_test[idx, :, channel], color=RED, lw=1.05, ls="--", alpha=0.9, label="shifted target Y" if position == 0 else None)
        ax.axvspan(tx[0], tx[-1], color=BLUE, alpha=0.05)
        ax.axvspan(ty[0], ty[-1], color=RED, alpha=0.05)
        ax.set_ylabel(data.channel_names[channel])
        ax.axhline(0, color=LIGHT_GRAY, lw=0.6)
    panel_label(axes[0], "A")
    axes[0].set_title(
        f"{data.provenance.get('dataset', 'MOABB')} historical one-sample-shift task: "
        f"{max(0, data.context_samples - data.forecast_horizon_samples)}/"
        f"{data.context_samples} target samples are already in the context"
    )
    axes[0].legend(loc="upper right", frameon=True)
    axes[-1].set_xlabel("Physical time from context start (s)")
    fig.supylabel(f"EEG amplitude ({data.signal_unit})")
    stamp_axis(axes[-1], data)
    save_figure(fig, out, "fig1_historical_context_target")


def figure_mentor_baselines_autocorr(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    """Render the baseline overlay and autocorrelation explanation as one figure."""

    idx = 0
    channel = channels[0]
    t = (np.arange(data.y_test.shape[1]) + data.forecast_horizon_samples) / data.sfreq
    fig, axes = plt.subplots(1, 2, figsize=(7.16, 3.2))
    fig.subplots_adjust(left=0.10, right=0.98, bottom=0.25, top=0.82, wspace=0.34)
    ax = axes[0]
    panel_label(ax, "A")
    ax.plot(t, data.y_test[idx, :, channel], color=BLACK, lw=1.35, label="observed shifted target")
    ax.plot(t, data.persistence_pred_test[idx, :, channel], color=BLUE, lw=1.05, label="persistence")
    ax.plot(t, data.y_pred_test[idx, :, channel], color=RED, lw=1.05, label="ridge")
    ax.text(
        0.02,
        0.05,
        "No provenance-matched Kahlus checkpoint\nor prediction export is available.",
        transform=ax.transAxes,
        fontsize=7,
        color=GRAY,
        va="bottom",
    )
    ax.set_title(f"Held-out example: {data.channel_names[channel]}", fontsize=9)
    ax.set_xlabel("Physical time from context start (s)")
    ax.set_ylabel(f"EEG ({data.signal_unit})")
    ax.legend(loc="upper right", fontsize=7, frameon=True)

    ax = axes[1]
    panel_label(ax, "B")
    max_lag = min(int(round(0.25 * data.sfreq)), data.x_train.shape[1] - 2)
    lags = np.arange(max_lag + 1) / data.sfreq
    autocorr = recordwise_autocorrelation(data.x_train, data.train_record_ids, max_lag=max_lag)
    for channel in channels:
        ax.plot(lags, autocorr[:, channel], lw=1.0, label=data.channel_names[channel])
    ax.axvline(data.forecast_horizon_samples / data.sfreq, color=RED, ls="--", lw=0.9, label="target offset")
    ax.set_title("Autocorrelation at the target offset", fontsize=9)
    ax.set_xlabel("Lag (s)")
    ax.set_ylabel("autocorrelation")
    ax.legend(fontsize=6.5, ncol=2, frameon=True)
    fig.text(
        0.5,
        0.04,
        "HISTORICAL OVERLAP DIAGNOSTIC - NOT CLAIM EVIDENCE. "
        "No provenance-matched Kahlus checkpoint is available.",
        ha="center",
        va="bottom",
        fontsize=6.5,
        color=GRAY,
    )
    save_figure(fig, out, "fig2_historical_baselines_autocorrelation")
...
def figure_prediction(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    ...
    t = (np.arange(y_true.shape[1]) + data.forecast_horizon_samples) / data.sfreq
    ...
    for ax, ch in zip(axes, channels):
        yt = y_true[idx, :, ch]
        yp = y_pred[idx, :, ch]
        pp = data.persistence_pred_test[idx, :, ch]
        r = _safe_corr(yt, yp)
        mse = float(np.mean((yt - yp) ** 2))
        ax.plot(t, yt, color=BLACK, lw=1.25, label="actual shifted target EEG")
        ax.plot(t, yp, color=RED, lw=1.05, alpha=0.88, label="ridge prediction")
        ax.plot(t, pp, color=BLUE, lw=0.95, alpha=0.78, label="persistence prediction")
    ...
    title = "Ridge and persistence overlay on the shifted target"
    title += " (historical Kahlus checkpoint unavailable)"
    axes[0].set_title(title)
    axes[-1].set_xlabel("Time into shifted target (s)")
    stamp_axis(axes[-1], data)
...
def figure_autocorr(data: RidgeFigureData, channels: list[int], out: Path) -> None:
    ...
    lags = np.arange(max_lag + 1) / data.sfreq
    panel_label(axes[0], "A")
    autocorr = recordwise_autocorrelation(x, data.train_record_ids, max_lag=max_lag)
    for ch in channels:
        axes[0].plot(lags, autocorr[:, ch], lw=1.05, label=data.channel_names[ch])
    axes[0].set_title(f"Per-channel lag structure (target offset {data.forecast_horizon_samples} samples)")
    ...
    axes[1].set_title("Context versus shifted-target channel correlation")
...
def write_readme(out: Path, data: RidgeFigureData, skipped: Iterable[str]) -> None:
    status = (
        "schematic demo figures, not benchmark evidence"
        if data.schematic
        else "generated from MOABB-preprocessed epoch export tensors"
    )
    ...
    out.joinpath("README.md").write_text(
        ...
        f"The plotted target begins {data.forecast_horizon_samples} sample(s) after the context starts; "
        f"it shares {max(0, data.context_samples - data.forecast_horizon_samples)}/{data.context_samples} samples with the context.\n\n"
        "## Generated files\n\n"
...
def main() -> None:
    ...
    ap.add_argument(
        "--out",
        type=Path,
        default=Path(tempfile.gettempdir()) / "ridge_eeg_figures",
        help="External output directory; paths inside Git repositories are rejected",
    )
    ...
    with fresh_atomic_output_directory(args.out) as out:
        ...
        if args.mentor_two_figures:
            figure_mentor_context_target(data, channels, out)
            figure_mentor_baselines_autocorr(data, channels, out)
            write_readme(out, data, ["only the two compact mentor-facing diagnostics were requested"])
        else:
            figure_input_target(data, channels, out)
            figure_prediction(data, channels, out)
            figure_autocorr(data, channels, out)
            figure_coefficients(data, channels, out)
            figure_psd_residual(data, channels, out)
            skipped = []
            topomap_skip = figure_topomap_if_possible(data, out, args.montage)
            if topomap_skip:
                skipped.append(topomap_skip)
            write_readme(out, data, skipped)
