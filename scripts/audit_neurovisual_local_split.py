#!/usr/bin/env python3
"""Audit an explicit NV-1 split manifest against a dataset registry JSON.

This validates split assignment leakage over user-provided manifest records. It
does not check raw file existence, download datasets, execute adapters, run
...
from neurotwin.neurovisual import validate_local_split_records  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--manifest", required=True, help="Path to a JSON list of local manifest records with split_name.")
    parser.add_argument("--registry", required=True, help="Path to a neurovisual dataset registry JSON.")
    args = parser.parse_args()

    manifest_path = Path(args.manifest)
    registry_path = Path(args.registry)
    payload = audit_local_split(manifest_path=manifest_path, registry_path=registry_path)
    print(json.dumps(payload, indent=2, sort_keys=True))
    return 0 if payload["passed"] else 1


def audit_local_split(*, manifest_path: Path, registry_path: Path) -> dict[str, Any]:
    failures: list[str] = []
    ...
    audit = validate_local_split_records(manifest_records, registry=registry_payload)
